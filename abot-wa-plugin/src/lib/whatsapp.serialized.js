...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  conn.sendMedia = async (jid, path, quoted, options = {}) => {
    ...
    return await conn.sendMessage(
      ...
      { quoted, ephemeralExpiration: global.ephemeral }
    );
  };
  ...
  conn.sendFile = async (
    ...
    let { status, file, filename, mime, size, extension } = await Func.getFile(
      url,
      name,
      opts && opts.referer ? opts.referer : false
    );
    if (!status) return conn.reply(jid, `Failed to proccess file.`, m);
    ...
    if (opts && opts.document) {
      ...
      if (/image\/(jpe?g|png)/.test(mime)) {
        ...
        return conn
          .sendMessage(
            jid,
            {
              ...
              caption: caption,
              mentions: [...caption.matchAll(/@(\d{0,16})/g)].map(
        ...
        await conn.sendPresenceUpdate(
          opts && opts.ptt ? "recoding" : "composing",
          jid
        );
        const process = await Func.metaAudio(file, {
          title: filename.replace(new RegExp(".mp3", "i"), ""),
  ...
  FN:${name.replace(/\n/g, "\\n")}
  ORG:
  item1.TEL;waid=${number}:${PhoneNumber("+" + number).getNumber(
      "international"
    )}
  ...
  conn.sendButtonDoc = async (
    ...
    const buttonMessage = {
      document: { url: "https://wa.me/6285752765133" },
      mimetype: global.doc,
  ...
  conn.send2ButtonImgDoc = async (
    ...
    if ((res && res.status !== 200) || file.length <= 200000) {
      try {
  ...
  conn.generateMessage = async (jid, message, quoted = {}, options = {}) => {
    ...
    return await conn
      .relayMessage(jid, generate.message, {
        messageId: generate.key.id,
      })
      .then(() => generate);
  };

  conn.sendMessageModify = async (
    jid,
    text,
    quoted,
    properties,
    ...
    return conn.generateMessage(
      jid,
      {
        ...
        contextInfo: {
          ...
          externalAdReply: {
            title: properties.title || "Simple WhatsApp Bot",
            body: properties.body || null,
            mediaType: 1,
            previewType: 0,
            showAdAttribution: properties.ads && properties.ads ? true : false,
            renderLargerThumbnail:
              properties.largeThumb && properties.largeThumb ? true : false,
            thumbnail:
              properties.thumbnail ||
              (await getBuffer("https://i.postimg.cc/T3hvPkww/BG-MENU.jpg")), // Buffer
            thumbnailUrl:
              "https://telegra.ph/?id=" +
              (Math.random(1000) * 1000000).toFixed(),
   ...
   * @param {Object} quoted
   */
  conn.send3TemplateButtonImg = async (
    jid,
    buffer,
    content,
    footerText,
  ...
  conn.send5TemplateButtonImg = async (
    jid,
    buffer,
    content,
    footerText,
    ...
    const buttons = [
      { index: 1, urlButton: { displayText: button1, url: id1 } },
      { index: 2, urlButton: { displayText: button2, url: id2 } },
      { index: 3, quickReplyButton: { displayText: button3, id: id3 } },
      { index: 4, quickReplyButton: { displayText: button4, id: id4 } },
      { index: 5, quickReplyButton: { displayText: button5, id: id5 } },
    ];
    const buttonMessage = {
      image: file,
      caption: content,
  ...
  conn.sendTemplateButtonDoc = async (
    jid,
    buffer,
    content,
    footerText,
  ...
  conn.sendTemplateButtonLoc = async (
    jid,
    buffer,
    contentText,
    footer,
  ...
  conn.send5TemplateButtonLoc = async (
    jid,
    buffer,
    contentText,
    footer,
        ...
        templateMessage: {
          hydratedTemplate: {
            ...
            hydratedButtons: [
              ...
              {
                quickReplyButton: {
                  displayText: buttons4,
                  id: row4,
                },
              },
              {
                quickReplyButton: {
                  displayText: buttons5,
                  id: row5,
                },
              },
            ],
  ...
  conn.sendTemplateButtonFakeImg = async (
    jid,
    buffer,
    content,
    footerText,
    ...
    const key = {
      ...
      templateButtons: [
        { index: 1, urlButton: { displayText: dtu, url: urlnya } },
        { index: 2, quickReplyButton: { displayText: btn1, id: id1 } },
      ],
      ...options,
    };
  ...
  conn.send2TemplateButtonFakeImg = async (
    jid,
    buffer,
    content,
    footerText,
    ...
    const key = {
      ...
      templateButtons: [
        { index: 1, urlButton: { displayText: dtu, url: urlnya } },
        { index: 2, quickReplyButton: { displayText: btn1, id: id1 } },
        { index: 3, quickReplyButton: { displayText: btn2, id: id2 } },
      ],
    };
    conn.sendMessage(jid, key, {
      quoted,
  ...
  conn.send3TemplateButtonFakeImg = async (
    jid,
    buffer,
    content,
    footerText,
    ...
    const key = {
      ...
      templateButtons: [
        ...
        { index: 2, quickReplyButton: { displayText: btn1, id: id1 } },
        { index: 3, quickReplyButton: { displayText: btn2, id: id2 } },
        { index: 4, quickReplyButton: { displayText: btn3, id: id3 } },
      ],
    };
    conn.sendMessage(jid, key, {
      quoted,
