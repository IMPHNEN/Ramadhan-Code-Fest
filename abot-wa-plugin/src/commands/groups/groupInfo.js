let handler = async (
  m,
  { conn, usedPrefix, participants, groupMetadata, text }
) => {
...
• *ID:* ${await groupMetadata.id}
• *Nama:* ${await groupMetadata.subject}
• *Deskripsi:* ${await groupMetadata.desc}
• *Total Anggota:* ${participants.length} Anggota
• *Pembuat Grup:* @${m.chat.split`-`[0]}
• *Admin Grup:* ${listAdmin}

*Pengaturan Bot:*
• *Anti Link:* ${antiLink ? "✅" : "❌"}
• *Anti Delete:* ${global.db.data.chats[m.chat].delete ? "❌" : "✅"}
• *Mute:* ${isBanned ? "✅" : "❌"}
• *Deskprisi:* ${descUpdate ? "✅" : "❌"}
• *Detect:* ${detect ? "✅" : "❌"}
• *Stiker:* ${stiker ? "✅" : "❌"}
• *Welcome:* ${welcome ? "✅" : "❌"}
• *Anti Spam:* ${antispam ? "✅" : "❌"}
• *Anti Badword:* ${antiBadword ? "❌" : "✅"}
• *Anti Troli:* ${antitroli ? "✅" : "❌"}
• *Anti Virtex:* ${antivirtex ? "✅" : "❌"}
• *Auto Tutup Grup:* ${closeGroup ? "✅" : "❌"}
...
`.trim();
    let mentionedJid = groupAdmins.concat([
      `${m.chat.split`-`[0]}@s.whatsapp.net`,
    ]);
    conn.sendMessageModify(m.key.remoteJid, caption, m, {
      ads: false,
      largeThumb: true,
