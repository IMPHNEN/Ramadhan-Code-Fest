...
let handler = async (
  m,
  { conn, args, usedPrefix, command, isPrems, isOwner, isROwner }
) => {
  ...
  if (!(isPrems || isOwner || isROwner)) {
    if (user.joincount === 0)
      throw `Kamu sudah melebihi token/limit memasukkan bot ke dalam grup!\nsilahkan membeli premium agar bisa memasukan bot kedalam grup lagi!`;
    user.joincount -= 1;
    ...
    conn.reply(m.chat, "Joining group...", m).then(async () => {
      ...
      conn.reply(
        m.chat,
        `Berhasil join grup ${await conn.getName(
          res
      ...
      for (let jid of global.owner
        ...
        if (data.exists)
          m.reply(
            `@${m.sender.split`@`[0]} telah menambahkan ${
              conn.user.name
            } ke ${await conn.getName(
              res
            )}\njid: ${res}, bot akan keluar dalam waktu: ${msToDate(
              global.db.data.chats[res].expired - now
            )}`.trim(),
            data.jid,
            { mentions: [m.sender] }
    ...
    if (!args[1])
      throw `Masukkan format yang benar! format: ${usedPrefix}join <link> <jumlah hari>`;
    let res = await conn.groupAcceptInvite(code);
    conn.reply(m.chat, "Oteeeweehhh...", m).then(async () => {
      ...
      await conn.reply(res, teks, m);
    });
  } else if (isPrems || !isOwner || !isROwner) {
    if (user.joincount === 0)
      throw `Kamu sudah melebihi token/limit memasukkan bot ke dalam group!`;
    user.joincount -= 1;
    let res = await conn.groupAcceptInvite(code);
    conn.reply(m.chat, "Joining group...", m).then(async () => {
