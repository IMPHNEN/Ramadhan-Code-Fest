  ...
  try {
    ...
    if (global.db.data.users[m.sender].healt > 79) {
      if (new Date() - global.db.data.users[m.sender].lastadventure > 300000) {
        ...
        let _diamond =
          rubah == 0
            ? pickRandom(["0", "1", "0", "1", "0", "1", "0"])
            : "" || rubah == 1
            ? pickRandom(["0", "1", "0", "1"])
            : "" || rubah == 2
            ? pickRandom(["0", "1", "0", "1", "2"])
            : "" || rubah == 3
            ? pickRandom(["0", "1", "0", "2", "2", "0"])
            : "" || rubah == 4
        ...
        let str = `
${rpg.emoticon("healt")} Nyawa mu berkurang -${
          healt * 1
        } karena Kamu telah berpetualang sampai ${pickRandom([
          "🇯🇵 Jepang",
          "🇰🇷 Korea",
          "🇮🇳 India",
          "🇺🇲 Amerika",
...
${rpg.emoticon("sampah")} *sampah:* ${sampah}${
          ...
          string == 0
            ? ""
            : `\n*${rpg.emoticon("string")}String:* ` + string + ""
        }${
          diamond == 0
            ? ""
...
`.trim();
        ...
        if (mythic > 0) {
          ...
          conn.reply(
            ...
            "*Selamat anda mendapatkan item Rare yaitu*\n" +
              mythic +
              `${rpg.emoticon("mythic")}Mythic Crate`,
            m
          );
        }
        if (legendary > 0) {
          ...
          conn.reply(
            ...
            "*Selamat anda mendapatkan item Epic yaitu*\n" +
              legendary +
              `${rpg.emoticon("legendary")}Legendary Crate`,
            m
          );
        }
        ...
        conn.reply(
          m.chat,
          `Anda sudah bekerja keras hari ini, silahkan menunggu sampai *${timers}*`,
          m
        );
    } else
      conn.reply(
        m.chat,
        "Minimal 80 health untuk bisa bercocok tanam, beli obat dulu biar kuat dengan ketik *" +
          usedPrefix +
          "shop buy potion <jumlah>*\ndan ketik *" +
          usedPrefix +
          "use potion <jumlah>*\n\n_Untuk mendapat money dan potion gratis ketik_ *" +
          usedPrefix +
          "claim*",
        m
      );
