  ...
  switch (type) {
    case "rubah":
      ...
      if (new Date() - global.db.data.users[m.sender].rubahlastclaim > 600000) {
        ...
        m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktur}* lagi`);
      break;
    case "kuda":
      ...
      if (new Date() - global.db.data.users[m.sender].kudalastclaim > 600000) {
        ...
        m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktuk}* lagi`);
      break;
    case "kucing":
      if (kucing == 0) return m.reply("*Kamu belum memiliki Pet Kucing*");
      if (kucing == 5) return m.reply("*Pet kamu dah lvl max*");
      let __waktul =
        new Date() - global.db.data.users[m.sender].kucinglastclaim;
      ...
      if (
        ...
        m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktul}* lagi`);
      break;
    case "anjing":
      ...
      if (
        new Date() - global.db.data.users[m.sender].anjinglastclaim >
        600000
      ) {
        if (global.db.data.users[m.sender].makananpet > 0) {
          global.db.data.users[m.sender].makananpet -= 1;
        ...
        m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktu}* lagi`);
      break;

    default:
      return conn.reply(
        m.chat,
        `${usedPrefix}feed [anjing | kucing | rubah | kuda]\nContoh penggunaan: *${usedPrefix}feed kucing*`,
        m
      );
  }
};
