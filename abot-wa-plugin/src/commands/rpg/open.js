let handler = async (m, { conn, command, args, text, usedPrefix }) => {
  try {
    ...
    let _lmao = args[0];
    let Lmao = `Hanya support 1, 10, 100, 1000\nContoh penggunaan: *${usedPrefix}open ${
      args > 2
        ? _lmao
        : pickRandom(["common", "uncommon", "mythic", "legendary"])
    } 10*`;
  ...
  `.trim();
      ...
      case "pet":
        ...
        if (global.db.data.users[m.sender].pet > 0) {
          ...
          if (_pet == "kucing" && kucing > 0) {
            ...
            conn.reply(
              m.chat,
              `Anda sudah memiliki pet ${_pet}, Hadiahmu diganti dengan 2 potion${
                mknp > 0 ? ` Dan ${mknp} Makanan Pet` : ""
              }`,
              m
            );
            ...
            conn.reply(
              m.chat,
              `*Selamat Anda mendapatkan pet${_pet} ${
                mknp > 0 ? ` Dan ${mknp} Makanan Pet*` : "*"
              }`,
              m
            );
    ...
    conn.reply(
      m.chat,
      `${usedPrefix}open <crate name> < 1 | 10 | 100 | 1000 >\n\nContoh penggunaan: *${usedPrefix}open common 10*\n\nlist crate:\n*common*\n*uncommon*\n*mythic*\n*legendary*`,
      m
    );
    if (DevMode) {
