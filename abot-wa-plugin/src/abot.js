const serialized = require("./lib/whatsapp.serialized");
...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      const isROwner = [global.conn.user.jid, ...global.owner]
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || "")[0])) {
          ...
          if (
            ...
            let user = global.db.data.users[m.sender];
            if (
              !["unbanchat.js", "dompet.js", "creator.js"].includes(name) &&
              chat &&
              chat?.isBanned &&
              !isPrems
            )
              return; // Kecuali ini, bisa digunakan
            if (
              !["unbanchat.js", "dompet.js", "creator.js"].includes(name) &&
              user &&
              user?.banned
          ...
          if (
            ...
            this.reply(
              m.chat,
              `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`,
      ...
      if (opts["queque"] && m.text && quequeIndex !== -1)
  ...
  async participantsUpdate({ id, participants, action, m }) {
    ...
    switch (action) {
              ...
              await this.sendMessageModify(id, text, null, {
                ...
                thumbnail: await getBuffer(
                  action === "add"
                    ...
                    : "https://telegra.ph/file/01001b9e5a863eefd8368.jpg"
                ),
              });
            }
          }
      ...
      case "demote":
        ...
        if (chat.detect) this.reply(id, text, m);
        break;
    }
  },
    ...
    for (let groupUpdate of groupsUpdate) {
      ...
      if (groupUpdate.restrict == true)
        text =
          ...
          this.sRestrictOn ||
          conn.sRestrictOn ||
          "```Group has been all participants!";
      if (groupUpdate.restrict == false)
        text =
          chats.sRestrictOff ||
          this.sRestrictOff ||
          conn.sRestrictOff ||
          "```Group has been only admin!";
      //console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
      if (!text) continue;
  ...
  async delete({ remoteJid, fromMe, id, participant }) {
    if (fromMe) return;
    let chats = Object.entries(await this.chats).find(
...
Hay, *${m.name}* 👋
Fitur ini hanya untuk premium bot!!`,
    group: `*GROUP ONLY*
 
Hay, *${m.name}* 👋
Fitur ini hanya untuk group bot!!`,
    private: `*PRIVATE CHAT ONLY*

Hay, *${m.name}* 👋
Fitur ini hanya dapat digunakan didalam privat chat!!`,
    admin: `*ADMIN ONLY*
...
contoh:
#register *${m.name}.umur*`,
    nsfw: `*NSFW NON AKTIF*

Hay, *${m.name}* 👋
Fitur ini dapat digunakan jika nsfw sudah aktif di grup!!`,
    rpg: `*RPG NON AKTIF*

Hay, *${m.name}* 👋
Fitur ini dapat digunakan jika rpg sudah aktif!!`,
